...
class Designer extends SwapPanel {
    ...
    private void getColour(int type) {
        if (type == 1)
        {
            ...
            backColour = colourChooser;
        }

        if (type == 2)
        {
            Color initColour = bufferBillboard.getInformationColour();
            Color colourChooser = JColorChooser.showDialog(this, "Choose Colour", initColour);
            bufferBillboard.setInformationColour(colourChooser);
            infoColour = colourChooser;
    ...
    protected void saveChanges() {
        ...
        if (backColour != null)
        {
            bufferBillboard.setBackgroundColour(backColour);
        }
        else
        {
            bufferBillboard.setBackgroundColour(Color.black);
        }
        ...
        if (information.getText().length()!=0)
        {
            if (infoColour != null)
            {
                bufferBillboard.setInformation(new Information(information.getText(), infoColour));
            }
            else
            {
        ...
        if (message.getText().length()!=0)
        {
            if (messColour != null)
            {
                bufferBillboard.setMessage(new Message(message.getText(), messColour));
            }
            else
            {
                bufferBillboard.setMessage(new Message(message.getText()));
            }
        }
    ...
    private class Message extends system.billboard.textfield.Message {
